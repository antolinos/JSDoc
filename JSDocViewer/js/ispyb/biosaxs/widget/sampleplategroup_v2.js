function SamplePlateGroupWidget(args){
	...
	this.heightPlates = this.height - this.titleTypePlate -55;
	this.margin = 0;
...
SamplePlateGroupWidget.prototype.getPlates = function(experiment){
	var plateGroups = experiment.getPlateGroups();
	...
	for ( var i = 0; i < plateGroups.length; i++) {
		...
		var id = plateGroups[i].plateGroupId;
		var plates = experiment.getPlatesByPlateGroupId(id);
		plates.sort(this._sortPlates);

		for ( var j = 0; j < plates.length; j++) {
			var td = document.createElement("td");
			td.setAttribute('id', this.id + "_" + plates[j].samplePlateId);
			...
			var text = document.createTextNode((j + 1) + ".- " + plates[j].platetype3VO.name);
...
SamplePlateGroupWidget.prototype.refresh = function(experiment){
	...
	if (document.getElementById(this.id + "_container") != null){
		...
		document.getElementById(this.id + "_container").innerHTML = this.getPlates(experiment);
		this.drawPlates(experiment);
	}
	/** We refrsh also the bbar * */
...
SamplePlateGroupWidget.prototype.getPanel = function(experiment){
	this.experiment = experiment;
	var _this = this;
		...
		items: [
		        {
		        	...
		        	html		: this.getPlates(experiment)
		        }
		        ]
	});
