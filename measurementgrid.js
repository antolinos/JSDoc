	...
	for ( var i = 0; i < measurements.length; i++) {
		...
		for ( var k = 0; k < measurementtodatacollection3VOs.length; k++) {
			if (measurementtodatacollection3VOs[k].dataCollectionOrder == 1) {
				var specimenBuffer = experiments
						.getSampleById(experiments.getMeasurementById(measurementtodatacollection3VOs[k].measurementId).specimenId);
		...
		fields : [
			{
				...
				type : 'numeric'
			}, 'volumeToLoad', 'transmission', 'viscosity', 'waitTime', 'flow', 'buffer_acronym', 'macromoleculeId', 'acronym', 'concentration',
			'extraFlowTime', 'volume', 'energy', 'expExposureTemperature', 'storageTemperature', 'timePerFrame', 'radiationAbsolute',
			'radiationRelative', 'status', 'comments' ],
...

							{
								...
								columns : [
									...
									},
									{
										...
										renderer : function(val, y, sample) {
											if (sample.raw.bufferSampleplateposition3VO != null) {
												return BIOSAXS.proposal.getBufferById(sample.raw.bufferId).acronym
														+ "<span style='font-style:oblique;'> Plate: ["
									...
									},
									{
										...
										renderer : function(val, y, sample) {
											if (_this.experiments != null) {
												return BUI.getSamplePositionHTML(_this.experiments.getSampleById(sample.raw.specimenId),
														_this.experiments.experiments[0]);
											}
										}
									} ]
							},
...
MeasurementGrid.prototype.test = function(targetId) {
	...
	var panel = measurementGrid.getPanel(measurementGrid.input().measurements, new ExperimentList(
			[ new Experiment(measurementGrid.input().experiment) ]));
